...
void start_test(boost::program_options::variables_map &vm)
{
    ...
    int    const report_interval = vm["report-interval"].as<int>();
    ...
    for (int i = 0; i < worker; i++)
        pool.emplace_back(
            [total_times, worker, bufsize, start, total_duration, singledist, zookeeper_host, clienti=i, report_interval, &counter] () mutable {
                ...
                try
                {
                    ...
                    for (int i = 0; i < total_times; i++)
                    {
                        ...
                        if (now - start > total_duration * 1s)
                        {
                            BOOST_LOG_TRIVIAL(info) << "Timeout (" << total_duration << "s). Closing client\n";
                            return;
                        }

                        if (clienti == worker - 1 && report_interval > 0 && (now - lastupdate > report_interval * 1s))
                        {
                            lastupdate = now;
                            BOOST_LOG_TRIVIAL(info) << "iops: " << counter - processed_request_count;
                            processed_request_count = counter;
                        }
