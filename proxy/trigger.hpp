...
namespace slsfs::trigger
{
    ...
    struct httphost
    {
        ...
        httphost(Poco::URI const &uriparser) : host{uriparser.getHost()},
    ...
    template <typename StreamType>
        requires IsSupportedStream<StreamType>
    ...
    public:
        ...
        trigger(net::io_context &io, std::string const &url, Arguments &&...args) : io_context_{io}, io_strand_{io}, resolver_{io},
    ...
    private:
        ...
        void start_connect(tcp::resolver::results_type results, std::shared_ptr<http::request<http::string_body>> req)
        {
            ...
            beast::get_lowest_layer(*stream_ptr_).async_connect(results, net::bind_executor(io_strand_, [self = this->shared_from_this(), req](beast::error_code ec, tcp::resolver::results_type::endpoint_type)
                                                                                            {
                                                                                                if (not ec)
                                                                                                {
    ...
    auto make_trigger(net::io_context &io, int max_func_count = 15)
