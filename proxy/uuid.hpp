...
namespace slsfs::uuid
{
    ...
    struct uuid : public pack::key_t
    {
        ...
        auto encode_base64() const -> std::string
        {
            ...
            for (char &c : raw_encode)
                if (c == '/')
                    c = '_';
            // std::cout << "Encoded data after encoding: " << raw_encode << std::endl;
            return raw_encode;
        }
    ...
    struct hash_compare
    {
        static auto hash(RangeType const &id) -> std::size_t
        {
        ...
        static bool equal(RangeType const &lhs, RangeType const &rhs)
        {
            return lhs == rhs;
        }
    };

    inline auto to_uuid(pack::key_t &key) -> uuid &
    {
        return *static_cast<uuid *>(&key);
    }

    inline auto to_uuid(pack::key_t const &key) -> uuid const &
    {
    ...
    {
        // std::cout << "Encoded data after encoding: " << up_cast(key).encode_base64() << std::endl;
